...
本附录集中列出正文反复使用的变换、对应规则与排序公式。除特别说明外，单模复振幅满足
$\alpha=(q+\ii p)/\sqrt2$，从而 $\comm{\hat q}{\hat p}=\ii$。

\section{Weyl 符号与反变换}

对一维正则坐标，算符 $\Ohat$ 的 Weyl 符号为
\begin{equation}
  O_W(q,p)=\int_{-\infty}^{\infty}\dd y\,
  \ee^{-\ii py/\hbar}
  \mel{q+y/2}{\Ohat}{q-y/2}.
\end{equation}
反变换写成
\begin{equation}
  \mel{x}{\Ohat}{x'}=
  ...
  \ee^{\ii p(x-x')/\hbar}
  O_W\!\left(\frac{x+x'}2,p\right).
\end{equation}
密度算符的 Wigner 函数采用
\begin{equation}
  W(q,p)=\frac{1}{2\pi\hbar}
  \int\dd y\,\ee^{-\ii py/\hbar}
  \mel{q+y/2}{\rhohat}{q-y/2},
\end{equation}
于是 $\int\dd q\dd p\,W=1$，且
\begin{equation}
...
\end{align}
其中
$\Lambda=\overleftarrow\partial_q\overrightarrow\partial_p-
\overleftarrow\partial_p\overrightarrow\partial_q$。Moyal 括号的低阶展开为
\begin{equation}
