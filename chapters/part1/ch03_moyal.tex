...
\begin{equation*}
  \hat A\leftrightarrow A_W(q,p),
  \qquad
  \hat B\leftrightarrow B_W(q,p),
\end{equation*}
则
\begin{equation}
...
\begin{example}
取 $A=q$、$B=p$。因为二阶以上导数全部为零，
\begin{equation}
  q\star p=qp+\frac{\ii\hbar}{2},
  \qquad
  p\star q=pq-\frac{\ii\hbar}{2}.
\end{equation}
因此 $(\comm{\hat q}{\hat p})_W=q\star p-p\star q=\ii\hbar$。不对易性没有消失，而是进入星积。
\end{example}
...
\begin{equation}
  ...
  \frac{\dd^{2\ell+1}V}{\dd q^{2\ell+1}}
  \frac{\partial^{2\ell+1}W}{\partial p^{2\ell+1}}.
  \label{eq:moyal-potential-series}
\end{equation}
当 $V$ 至多为二次多项式时，$V^{(3)}$ 及更高导数全部为零，精确量子传播便与经典 Liouville 传播一致。准确条件是 Hamiltonian 的 Weyl 符号对全部相空间变量至多二次，而不只是“势能看起来像抛物线”。

\section{非线性怎样产生高阶导数}

考虑四次势
\begin{equation}
  V(q)=\frac{\lambda}{4}q^4.
\end{equation}
因为 $V'''(q)=6\lambda q$ 且五阶以上导数为零，精确方程为
\begin{equation}
  \pder{W}{t}
  =-\frac{p}{m}\pder{W}{q}
  +\lambda q^3\pder{W}{p}
  -\frac{\hbar^2\lambda q}{4}\frac{\partial^3W}{\partial p^3}.
...
\begin{align}
  \hat q\rhohat&\leftrightarrow
  \left(q+\frac{\ii\hbar}{2}\partial_p\right)W,
  &\rhohat\hat q&\leftrightarrow
  \left(q-\frac{\ii\hbar}{2}\partial_p\right)W,\\
  \hat p\rhohat&\leftrightarrow
  \left(p-\frac{\ii\hbar}{2}\partial_q\right)W,
  &\rhohat\hat p&\leftrightarrow
  \left(p+\frac{\ii\hbar}{2}\partial_q\right)W.
  \label{eq:bopp-real}
\end{align}
...
\end{align}
把 Hamiltonian 或 Lindblad 主方程左右两侧的算符依次替换为\cref{eq:bopp-real,eq:bopp-complex}，即可得到 Wigner 偏微分方程。
...
高阶导数对低阶多项式矩的直接贡献可能因分部积分而消失。例如\cref{eq:quartic-wigner-evolution} 中的三阶 $p$ 导数不直接改变 $p^0,p^1,p^2$ 矩。然而，非线性会使低阶矩与更高阶矩耦合，高阶结构随后仍可能反馈回来。因此，某个低阶观测量在一段时间内准确，不代表完整 Wigner 函数或所有高阶关联都同样准确。

\begin{chaptercheck}
看到“经典轨迹”时，检查它来自精确的二次 Hamiltonian，还是来自对高阶 Moyal 导数的截断；看到“随机轨迹”时，检查噪声只在初态出现，还是由二阶扩散在演化中持续产生。
\end{chaptercheck}

\section*{本章小结}

星积保存了算符乘法和不对易性，Moyal 括号把 von Neumann 方程精确转写为相空间演化。Poisson 括号是其领先项，非线性 Hamiltonian 产生高阶导数。TWA 的核心数学操作正是舍弃无法由普通轨迹表示的高阶项，而 Bopp 算符提供了对具体玻色 Hamiltonian 进行这一推导的实用途径。

\begin{exercises}
  \item 使用\cref{eq:star-product} 计算 $q^2\star p^2$，保留全部非零项。
  \item 证明 Moyal 括号满足反对称性，并验证 $\{q,p\}_{\mathrm{M}}=1$。
  \item 从\cref{eq:moyal-potential-series} 推导四次势的\cref{eq:quartic-wigner-evolution}。
  \item 使用 Bopp 规则重新推导 $(\had\ha)_W=|\alpha|^2-1/2$。
  \item 对六次势 $V(q)=gq^6/6$，写出 Wigner 演化中最高阶的 $p$ 导数。
\end{exercises}
