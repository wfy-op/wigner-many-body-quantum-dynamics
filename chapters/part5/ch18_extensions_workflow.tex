...
式\eqref{eq:wigner-linear-loss-sde}是本章完整推导范围内的代表例。此处线性损耗给出加性噪声，所以 It\^o 与 Stratonovich 漂移相同。对场损耗，$\dd W(x)$ 也必须投影到与哈密顿量相同的有限模式子空间；在格点上直接给每点独立噪声等价于选择特定投影和截止，不能在连续极限中保持噪声幅度不变而不重新检查单位。

非线性跳跃算符或非线性哈密顿量仍可能产生三阶及更高导数。例如两体损耗 $\hat L\propto\ha^2$ 会产生乘性扩散及不能自动视为 Fokker--Planck 项的高阶导数。此时把方程截为二阶是开放系统 TWA，既含 Hamilton 截断误差，也含耗散通道截断误差；漂移、扩散和排序修正必须从指定主方程逐项推导，不能把线性损耗公式中的 $\kappa$ 换成密度相关系数。

还必须声明随机微分方程采用 It\^o 还是 Stratonovich 解释。对独立实 Wiener 增量满足 $\dd W_j\dd W_l=\delta_{jl}\dd t$ 的方程
\begin{equation}
  \dd X_i=A_i^{(I)}\dd t+\sum_jB_{ij}(\bm X)\dd W_j,
\end{equation}
等价的 Stratonovich 漂移为
\begin{equation}
  A_i^{(S)}=A_i^{(I)}-\frac12\sum_{k,j}B_{kj}\partial_{X_k}B_{ij}.
  \label{eq:ito-stratonovich-drift-conversion}
\end{equation}
复噪声应先拆成声明了协方差的实分量再应用此式。扩散矩阵的分解 $D=BB^T$ 不唯一，不同分解可以改变轨迹方差却不改变理想 Fokker--Planck 方程。

\begin{chaptercheck}
本章只完整推导线性单模损耗，\cref{eq:wigner-linear-loss-sde} 是验算例。非线性损耗、驱动耗散稳态和开放多模场尚无通用实现。若研究结论依赖非线性跳跃，必须另给从 Lindblad 项到漂移、扩散及高阶余项的推导，并声明随机积分约定、投影噪声和一个可解开放系统基准。
\end{chaptercheck}
...
$\beta=\alpha^*$。对许多最多四次的玻色哈密顿量，主方程可精确映射为二阶 Fokker--Planck 方程，因此原则上没有 TWA 的 Moyal 截断。代价是轨迹权重和变量可能形成长尾，抽样方差随时间爆炸，边界项也需诊断。它是“动力学表示更精确但统计窗口可能更短”的方法，而不是无条件优于 TWA。
这一加倍相空间构造的经典来源见文献\cite{Drummond1980}。
本节只说明选择方法时的边界，不给出 positive-P 的通用数值处方；实际使用还需为具体哈密顿量导出两套独立变量的漂移、选择扩散分解，并监测移动奇点、边界项与抽样尾部。
...
\begin{enumerate}
  \item \textbf{模型冻结}：写明哈密顿量、维数、边界、单位和模式投影；
...
给定初态 $\rhohat_0$、哈密顿量 $\Hhat$ 和观测量 $\Ohat$，一个可审计的计算闭环应当是：
\begin{enumerate}
  \item 固定有限模式模型并求 $H_W$；
...
在一个明确受限的均匀 Rabi 双分量一维模型中实际走完了“旧 BdG 基抽样--双场传播--排序修正观测量--步长与轨迹误差”这条链，并加入三档独立抽样的有限截止窗压力测试。它是集成基准，不是完整研究结论：尚未覆盖 Raman SOC 传播、高精度截止与尺寸外推、平衡超流刚度、开放系统或独立多体方法交叉验证。因而本书提供的是一套闭环\emph{模板}及一个可运行实例，而不是已经验证所有目标模型的通用生产管线。
...
开放系统的 Wigner 方程可以包含物理扩散噪声，非线性跳跃仍可能需要额外截断；本章只完整覆盖线性损耗示例。positive-P、张量网络、精确对角化和量子动理学保存不同的量子信息，应按目标主张组合使用。可靠研究从可证伪结论反推模型、基准、收敛和元数据，并设置明确停止条件。至此，全书给出了从 Wigner 基础、TWA 推导、玻色场实现到双分量淬火诊断的闭环方法模板，并用一个均匀 Rabi 空间场实例验证了其中的主要接口；Raman SOC、平衡超固态与开放场论仍是需要继续完成独立验证的扩展，而不是本书已经数值证明的结论。
