...
\end{equation}
以一维接触相互作用为例，二次量子化 Hamilton 量写成
\begin{equation}
...
在长度 $L$ 的周期区间上取 $M$ 个格点，$\Delta x=L/M$。定义格点模式
\begin{equation}
  \ha_j=\sqrt{\Delta x}\,\hpsi(x_j),
  \qquad
...
\begin{warningbox}
若代码输出 $\Delta x\sum_j|\psi_j|^2$ 而没有减去 $M/2$，它输出的是对称排序范数，不是物理粒子数。这个差异在 $N\gg M$ 时可能相对很小，却会系统污染耗尽量、低密度尾部和模式占据。
\end{warningbox}

\section{离散 Hamilton 量与 Weyl 漂移}
