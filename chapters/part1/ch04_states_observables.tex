...
\begin{enumerate}
  ...
  \item 把等时正常排序观测量转换为 Wigner 样本估计量。
\end{enumerate}
...
\end{align}
采用 $\comm{\hat q_j}{\hat p_k}=\ii\delta_{jk}$ 时，物理协方差必须满足
\begin{equation}
...
\end{equation}
给出直接抽样。$L$ 可以取 Cholesky 因子；当 $V$ 半正定但数值上接近奇异时，可使用特征分解并设置与机器精度相容的容差。

\subsection{单模压缩真空}

若压缩轴与 $q$ 轴重合，实压缩参数为 $r$，则
\begin{equation}
  V=\frac12
  \begin{pmatrix}
    \ee^{-2r}&0\\
...
\end{equation}
其中 $L_n$ 是 Laguerre 多项式。除真空外，分布通常含正负交替的环。普通 Monte Carlo 需要非负抽样权重，因此不能把\cref{eq:fock-wigner} 直接当作概率密度。

实际工作中有三种选择：使用能够处理复权重或扩展相空间的表示；构造只匹配目标低阶矩的近似正分布；或者在小系统中直接使用精确态作为基准。第二种选择不是精确表示，必须说明匹配了哪些矩，以及结论是否对未匹配的高阶矩敏感。
...
表示 & 典型排序 & 主要特征 \\
\midrule
Glauber--Sudarshan $P$ & 正常排序 & 可能比普通函数更奇异 \\
Wigner $W$ & 对称排序 & 实函数，但可为负 \\
Husimi $Q$ & 反正常排序 & 始终非负，但额外平滑 \\
\bottomrule
\end{tabular}
\end{center}
...
相干态和热态可以逐模直接抽样，一般高斯态应由完整协方差矩阵抽样。压缩态说明正 Wigner 函数仍能代表量子态，Fock 态和猫态则展示普通概率抽样的限制。等时观测量必须从 Weyl 符号构造；正常排序、多时排序和连续场极限需要分别处理。
