...
\end{equation}
若 Hamilton 量保持平移对称，而有限轨迹在随机位置形成条纹，系综平均 $\ensavg{\rho_Q}$ 可以因随机相位为零。此时
...
配套程序分别抽样平均占据为 $4$ 的相干态和平均占据为 $2$ 的热态，用同一批轨迹计算密度、四阶正规排序矩与 $g^{(2)}$：
\begin{center}
...
\begin{exercises}
  ...
  \item 证明式\eqref{eq:unitary-discrete-fourier}保持总对称排序范数。
